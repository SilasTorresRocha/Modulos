...
*   **Single-Core (Prioridades de Task):** O ESP32-S2 possui apenas um núcleo (Xtensa LX7). Portanto, não é possível separar a rede em um núcleo e a tela em outro. A solução arquitetural é usar as prioridades do FreeRTOS:
    *   `Tarefa_Roteamento (Prioridade Alta - ex: 3)`: Roda o `scRoteadorBridgeM3` e o `scGestorRede`. Lida com MQTT e ESP-NOW de forma prioritária.
    *   `Tarefa_AtualizarTela (Prioridade Baixa - ex: 1)`: Exclusiva para o loop do LVGL (`lv_timer_handler()`). Se o LVGL engasgar carregando uma imagem, o FreeRTOS pausa a tela brutalmente, processa a emergência (gás do M1) na tarefa de rede, e devolve o controle depois.
*   **O Pulo do Gato (DMA):** A biblioteca `TFT_eSPI` empurra o buffer de RAM gerado pelo LVGL diretamente para a tela via SPI DMA (Direct Memory Access). Os bytes viajam sozinhos pelo barramento, poupando a CPU para lidar com a rede.

## Hardware Necessário
*   **ESP32-S2** (Mais GPIOs disponíveis e alta alocação de memória RAM/PSRAM).
*   **Tela Touch ILI9488 3.5" (SPI 320x480)**.
*   **SD Card SPI** (Sistema de arquivos local para salvar ícones, papéis de parede e cache da interface UI). O banco de dados offline usará o formato **JSONL (JSON Lines)**, que permite gravar o histórico via `.println()` e ler uma linha por vez usando `file.readStringUntil('\n')`, evitando estouro de Heap (vazamento de RAM) ao processar megabytes de dados quando a internet retornar.

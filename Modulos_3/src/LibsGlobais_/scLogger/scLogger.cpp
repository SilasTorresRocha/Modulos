...
void scLogger::inicializar(bool outputSerial, scRelogioSincronizado *relogio,
...
void scLogger::registrar(NivelLog nivel, String origem, String mensagem) {
  ...
  if (nivel == LOG_ERROR) {
    ...
    // Se falhou (sem internet ou MQTT fora) e temos o LittleFS, guarda
    // fisicamente
...
void scLogger::despacharErrosPendentes() {
  ...
  // Pega todas as chaves salvas que comecam com "crash_" (separadas por
  // virgula)
  String chavesPulo = _armazenamento->listarChavesComPrefixo("crash_");
  if (chavesPulo.length() == 0)
    return;

  // Divisao rudimentar da string separada por virgula (simulacao)
  int inicioIndex = 0;
  while (inicioIndex < chavesPulo.length()) {
    int indexVirgula = chavesPulo.indexOf(',', inicioIndex);
    if (indexVirgula == -1)
      indexVirgula = chavesPulo.length();

    String chave = chavesPulo.substring(inicioIndex, indexVirgula);
...
String scLogger::formatarMensagem(NivelLog nivel, String origem,
                                  String mensagem) {
  String prefixo = "";
  if (nivel == LOG_INFO)
    prefixo = "[INFO]";
  else if (nivel == LOG_WARNING)
    prefixo = "[WARN]";
  else if (nivel == LOG_ERROR)
    prefixo = "[ERR]";

...
void scDespachanteComandos::registrarCallbackPromiscuo(CallbackPromiscuoLocal callback) {
    _callbackPromiscuo = callback;
}
...
void scDespachanteComandos::processarPayload(const char* macOrigemTransceptor, const char* payload) {
    ...
    if (comandoStr == "status_peer") {
        if (_logger) _logger->info("DESPACHANTE", "Notificacao de morte/status de peer recebida.");
        // Obrigatorio repassar para o .ino avisar a UI (scControladorMenu) para ocultar menus que dependem dele
        if (_callbackLocal != nullptr) {
            _callbackLocal(cmd, args);
        }
        return;
